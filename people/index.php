<?php
session_start();
$page_title = 'Actor Entity Editor';
...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed Actor People List");
	echo ("<center>");
	echo ("<h3>Actor Entity Editor</h3><p>");
	echo ("<a href=add.php>Add Actor Entity</a><p>");
	...
	echo "Total Actor Entities - $records <br>";
	...
	$query = "SELECT * FROM person LIMIT $start, $display";
	$result = query($query);


	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1 class=table-list><tr>
	<th>First Name</th><th>Last Name</th><th>*</th><th>*</th></tr>";
	...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		...
		echo "<td><a href=deleteconfirm.php?id=" . $row['person_id'] . ">Delete</a></td>";
		echo "<td><a href=update.php?id=" . $row['person_id'] . ">Update</a></td></tr>";
