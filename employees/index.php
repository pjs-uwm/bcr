...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed Employee List");
	echo ("<center>");
	echo ("<h3>Employee Editor</h3><p>");
	echo ("<a href=add.php>Add Employee</a><p>");
	...
	echo "Total Employees - $records <br>";
	...
	$query = "SELECT e.*, u.first_name, u.last_name FROM employees e LEFT OUTER JOIN users u on e.user_id = u.user_id LIMIT $start, $display";
	$result = query($query);


	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1 class=table-list><tr>
	<th>Employee ID</th><th>Employee Name</th><th>*</th><th>*</th></tr>";
	...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		...
		echo "<td><a href=deleteconfirm.php?id=" . $row['employee_id'] . ">Delete</a></td>";
		echo "<td><a href=update.php?id=" . $row['employee_id'] . ">Update</a></td></tr>";
