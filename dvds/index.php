...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed Physical DVD List");
	echo ("<center>");
	echo ("<h3>Physical DVD Assets</h3><p>");
	echo ("<a href=add.php>Add DVD Asset</a><p>");
	//Set the number of records to display per page
	$display = 5;
	//Count the number of records;
	$query = 'SELECT COUNT(dvd_id) FROM dvds where is_lost is null';
	$result = query($query);
	$row = @mysqli_fetch_array($result, MYSQLI_NUM);
	$records = $row[0]; //get the number of records	

	echo "Total Physical DVD Assets - $records <br>";
	...
	$query = "SELECT * FROM dvds WHERE is_lost is null LIMIT $start, $display";
	$result = query($query);


	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1 class=table-list><tr>
	<th>DVD Serial Number</th><th>Date Received</th><th>*</th><th>*</th></tr>";

	//Fetch and print all the records...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		echo "<tr><td>" . $row['dvd_serial_number'] . "</td>";
		echo "<td>" . $row['date_received'] . "</td>";
		// hide links longer than 20 characters to not mess up table flow		 
		echo "<td><a href=deleteconfirm.php?id=" . $row['dvd_id'] . ">Delete</a></td>";
		echo "<td><a href=update.php?id=" . $row['dvd_id'] . ">Update</a></td></tr>";
