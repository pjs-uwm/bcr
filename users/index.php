...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed User List");
	echo ("<center>");
	echo ("<h3>User Editor</h3><p>");
	if ($_SESSION['role_id'] <> $ROLE_CUSTOMER) {
		echo ("<a href=add.php>Add User</a><p>");
	}
	//Set the number of records to display per page
	$display = 10;
	//Count the number of records;
	if ($_SESSION['role_id'] == $ROLE_CUSTOMER) {
		$query = 'SELECT COUNT(user_id) FROM users WHERE user_id = ' . $_SESSION['user_id'];
	} else {
	...
	if ($_SESSION['role_id'] == $ROLE_CUSTOMER) {
		$query = "SELECT * FROM users WHERE user_id = " . $_SESSION['user_id'] . " LIMIT $start, $display";
	} else {
	...
	<th>User Name</th><th>User Email Address</th><th>*</th>";
	if ($_SESSION['role_id'] != $ROLE_CUSTOMER) {
		echo "<th>*</th>";
	}
	echo "</tr>";
	...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		...
		if ($_SESSION['role_id'] != $ROLE_CUSTOMER) {
			echo "<td><a href=deleteconfirm.php?id=" . $row['user_id'] . ">Delete</a></td>";
		}
		echo "<td><a href=update.php?id=" . $row['user_id'] . ">Update</a></td></tr>";
