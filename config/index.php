...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed Application Configuration");
	echo ("<center>");
	echo ("<h3>Configuration Items</h3><p>");
	echo ("<a href=add.php>Add Configuration Item</a><p>");
	//Set the number of records to display per page
	$display = 10;
	//Count the number of records;
	$query = 'SELECT COUNT(config_item) as total_items FROM config_items';
	$result = query_arr($query);
	$records = $result[0]["total_items"]; //get the number of records	

	echo "Total Configuration Items - $records <br>";
	...
	$query = "SELECT * FROM config_items LIMIT $start, $display";
	$result = query($query);


	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1 class=table-list><tr>
	<th>Configuration Item</th><th>Configuration Value</th><th>*</th><th>*</th></tr>";

	//Fetch and print all the records...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		echo "<tr><td>" . $row['config_item'] . "</td>";
		echo "<td>" . $row['config_value'] . "</td>";
		echo "<td><a href=deleteconfirm.php?id=" . $row['config_id'] . ">Delete</a></td>";
		echo "<td><a href=update.php?id=" . $row['config_id'] . ">Update</a></td></tr>";
