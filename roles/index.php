...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed Role List");
	echo ("<center>");
	echo ("<h3>Roles</h3><p>");
	echo ("<a href=add.php>Add Role</a><p>");
	//Set the number of records to display per page
	$display = 5;
	//Count the number of records;
	$query = 'SELECT COUNT(role_id) FROM roles';
	$result = query($query);
	$row = @mysqli_fetch_array($result, MYSQLI_NUM);
	$records = $row[0]; //get the number of records	

	echo "Total Roles - $records <br>";
	...
	$query = "SELECT * FROM roles LIMIT $start, $display";
	$result = query($query);


	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1 class=table-list><tr>
	<th>Role ID</th><th>Role Name</th><th>*</th><th>*</th></tr>";

	//Fetch and print all the records...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		echo "<tr><td>" . $row['role_id'] . "</td>";
		echo "<td>" . $row['role_description'] . "</td>";
		// hide links longer than 20 characters to not mess up table flow		 
		echo "<td><a href=deleteconfirm.php?id=" . $row['role_id'] . ">Delete</a></td>";
		echo "<td><a href=update.php?id=" . $row['role_id'] . ">Update</a></td></tr>";
