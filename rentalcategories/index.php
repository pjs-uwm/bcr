<?php
session_start();
$page_title = 'Rental Category Editor';
...
if (!isset($_SESSION['email'])) {
	...
	log_event($_SESSION['user_id'], "Viewed Rental Category List");
	echo ("<center>");
	echo ("<h3>Rental Categories</h3><p>");
	echo ("<a href=add.php>Add Rental Category</a><p>");
	//Set the number of records to display per page
	$display = 5;
	//Count the number of records;
	$query = 'SELECT COUNT(rental_category_id) FROM rental_category';
	$result = query($query);
	$row = @mysqli_fetch_array($result, MYSQLI_NUM);
	$records = $row[0]; //get the number of records	

	echo "Total Rental Categories - $records <br>";
	...
	$query = "SELECT * FROM rental_category LIMIT $start, $display";
	$result = query($query);


	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1 class=table-list><tr>
	<th>Rental Category ID</th><th>Rental Category</th><th>*</th><th>*</th></tr>";

	//Fetch and print all the records...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
		echo "<tr><td>" . $row['rental_category_id'] . "</td>";
		echo "<td>" . $row['rental_category'] . "</td>";
		// hide links longer than 20 characters to not mess up table flow		 
		echo "<td><a href=deleteconfirm.php?id=" . $row['rental_category_id'] . ">Delete</a></td>";
		echo "<td><a href=update.php?id=" . $row['rental_category_id'] . ">Update</a></td></tr>";
